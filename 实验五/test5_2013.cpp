	...
	if (loginsuc == 1)  /*Login Successfully*/
	{
		while (1)
		{
			...
			if (choiceend == 1)
			{
				...
				printf("Bad command or file name.\nC:\\%s>", _strupr(username));
			gets(choice);
			strcpy(choice, ltrim(rtrim(_strlwr(choice))));
...
void LoginF()  /*LOGIN FileSystem*/
{
	...
	while (1)
	{
		...
		fp_mfd = fopen("c:\\osfile\\mfd", "rb");//c:\\osfile\\mfd
		for (i = 0; fread(&ufd_lp, sizeof(OSUFD_LOGIN), 1, fp_mfd) != 0; i++)
		if (strcmp(_strupr(ufd_lp.ufdname), _strupr(loginame)) == 0)
		{
			findout = 1;
		...
		if (findout == 1)  /*user exist*/
		{
			...
			{
				strcpy(username, _strupr(loginame));
				strcpy(dirname, username);
				fp_mfd = fopen("c:\\osfile\\mfd", "rb");//mfd
				for (j = 0; fread(&ufd_lp, sizeof(OSUFD_LOGIN), 1, fp_mfd) != 0; j++)
				{
					strcpy(str, "c:\\osfile\\mfd");//\\mfd
					strcat(str, ufd_lp.ufdname);
					ufd[j] = (OSUFD*)malloc(sizeof(OSUFD));
					strcpy(ufd[j]->ufdname, _strupr(ufd_lp.ufdname));
					fp_ufd = fopen(str, "rb");
					fcount[j] = 0;
			...
			else
			{
				...
				while (flag)
				{
					...
					gets(a);
					ltrim(rtrim(a));
					if (strcmp(_strupr(a), "Y") == 0) { loginsuc = 0; flag = 0; }
					else if (strcmp(_strupr(a), "N") == 0){ loginsuc = 0; flag = 0; return; }
				}
			}
		}
		else
		{
			...
			if (strcmp(loginpw, logincpw) == 0)
			{
				//将输入的注册名和密码保存到结构体变量ufd_lp中
				strcpy(ufd_lp.ufdname, _strupr(loginame));
				strcpy(ufd_lp.ufdpword, loginpw);
				//将结构体变量ufd_lp中中的注册名和密码写入到mfd文件中
				fp_mfd = fopen("c:\\osfile\\mfd", "ab+");
				fwrite(&ufd_lp, sizeof(OSUFD_LOGIN), 1, fp_mfd);
				fclose(fp_mfd);
				//新建一个由mfd和注册名的大写字母作为文件名的文件
				strcpy(username, _strupr(loginame));
...
void DirF()  /*Dir FileSystem*/
{
	...
	if (strcmp(_strupr(ltrim(rtrim(dirname))), "") != 0)
	{
		printf("\n\nC:\\%s>dir\n", dirname);
		...
		for (i = 0; i<fcount[j]; i++)
		{
			...
			_itoa(ufd[j]->ufdfile[i].fpaddr, str, 10);
			strcpy(sfpaddr, "file");
			strcat(sfpaddr, str);
...
int ExistD(char *dirname)  /*Whether DirName Exist,Exist-i,Not Exist-0*/
{
	...
	int exist = 0;
	for (i = 0; i<ucount; i++)
	if (strcmp(_strupr(ufd[i]->ufdname), _strupr(dirname)) == 0)
	{
		exist = 1;
		break;
	}
...
void CdF() /*Exchange Dir*/
{
	...
	gets(dname);
	ltrim(rtrim(dname));
	if (ExistD(dname) >= 0)  strcpy(dirname, _strupr(dname));
	else if (strcmp(_strupr(dname), "CD..") == 0)  strcpy(ltrim(rtrim(dirname)), "");
...
void CreateF()  /*Create File*/
{
	...
	if (strcmp(_strupr(dirname), _strupr(username)) != 0)
	{
			...
			if ((strcmp(fmode, "0") == 0) || (strcmp(fmode, "1") == 0) || (strcmp(fmode, "2") == 0) || (strcmp(fmode, "3") == 0))
			{
				...
				if (fpaddrno >= 0)
				{
					...
					while (flag)
					{
						...
						if (strcmp(_strupr(a), "Y") == 0)
						{
							fp_file = fopen(str, "wb+");
							ufd[i]->ufdfile[fcount[i] - 1].flength = WriteF1();
							flag = 0;
						}
						else if (strcmp(_strupr(a), "N") == 0){ flag = 0; wgetchar = 1; }
					}
...
int ExistF(char *filename)  /*Whether FileName Exist,Exist-i,Not Exist-0*/
{
	...
	j = ExistD(dirname);
	for (i = 0; i<fcount[j]; i++)
	if (strcmp(_strupr(ufd[j]->ufdfile[i].fname), _strupr(filename)) == 0)
	{
		exist = 1;
		break;
	}
			...
			else
			{
				while (flag)
				{
					...
					if (strcmp(_strupr(a), "Y") == 0)
					{
						...
						_itoa(ufd[k]->ufdfile[i].fpaddr, str, 10);
...
void ModifyFM() /*Modify FileMode*/
{
	...
	int ExistF(char *filename);  /*Whether FileName Exist,Exist-i,Not Exist-0*/
	int ExistD(char *dirname);
	if (strcmp(_strupr(dirname), _strupr(username)) != 0) { printf("\nError.You can only modify filemode in yourself dir.\n"); wgetchar = 1; }
	else
	{
		printf("\nPlease input FileName:");
		gets(fname);
...
void ReadF() /*Read File*/
{
	...
	int ExistF(char *filename);  /*Whether FileName Exist,Exist-i,Not Exist-0*/
	int ExistD(char *dirname);
	if (strcmp(_strupr(ltrim(rtrim(dirname))), "") == 0) { printf("\nError.Please convert to ufd dir before read.\n"); wgetchar = 1; return; }
